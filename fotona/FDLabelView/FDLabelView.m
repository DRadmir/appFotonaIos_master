...
-(void)drawDebug:(NSArray*)linebreaks{
    ...
    if (debug) {
        ...
        if (debugShowCoordinates) {
            ...
            CGContextFillRect(ctx, CGRectMake(0, 0, [info sizeWithAttributes:@{NSFontAttributeName: debugFont}].width, debugHeight));
            
            CGContextSetRGBFillColor(ctx, 1, 1, 1, 1);
            [info drawAtPoint:CGPointZero withAttributes:@{NSFontAttributeName: debugFont}];
            
            // center point
            info = [NSString stringWithFormat:@"%.1f, %.1f", self.frame.origin.x + center.x, self.frame.origin.y + center.y];
            
            CGContextSetRGBFillColor(ctx, 0, 0, 0, 0.8);
            CGContextFillRect(ctx, CGRectMake(center.x, center.y - debugHeight, [info sizeWithAttributes:@{NSFontAttributeName: debugFont}].width, debugHeight));
            
            CGContextSetRGBFillColor(ctx, 1, 1, 1, 1);
            [info drawAtPoint:CGPointMake(center.x, center.y - debugHeight) withAttributes:@{NSFontAttributeName: debugFont}];
            
            // size
            info = [NSString stringWithFormat:@"F %.1f W %.1f H %.1f", displayFont.pointSize, self.frame.size.width, self.frame.size.height];
            
            CGFloat infoWidth = [info sizeWithAttributes:@{NSFontAttributeName: debugFont}].width;
            CGContextSetRGBFillColor(ctx, 0, 0, 0, 0.8);
            CGContextFillRect(ctx, CGRectMake(center.x - infoWidth * 0.5, self.bounds.size.height - debugHeight, infoWidth, debugHeight));
            
            CGContextSetRGBFillColor(ctx, 1, 1, 1, 1);
            [info drawAtPoint:CGPointMake(center.x - infoWidth * 0.5, self.bounds.size.height - debugHeight) withAttributes:@{NSFontAttributeName: debugFont}];
        }
    }
...
-(CGFloat)drawWord:(NSString*)word pos:(CGPoint)pos{
    ...
    for (int i = 0; i < word.length; i++) {
        s = [word substringWithRange:NSMakeRange(i, 1)];
        width = [s sizeWithAttributes:@{NSFontAttributeName: displayFont, }].width;
        [s drawAtPoint:CGPointMake(pos.x, pos.y + offsetY) withAttributes:@{NSFontAttributeName: displayFont, NSForegroundColorAttributeName:self.textColor}];
        pos.x += width;
